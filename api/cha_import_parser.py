"""Parser server-side, no destructivo, de los ficheros de importación CHA."""
...
def _header(rows: list[tuple[int, list[Any]]], required: list[str], aliases: dict[str, set[str]] | None = None):
    aliases = aliases or {}
...
def _metadata(stream, filename: str, sheets: dict[str, list[tuple[int, list[Any]]]], detected: str):
    stream.seek(0)
    content = stream.read()
...
def detectar_tipo(sheets: dict[str, list[tuple[int, list[Any]]]) -> str:
...
def parse_file(stream, filename: str) -> tuple[str, dict[str, Any]]:
...
def _parse_original(sheets):
    ...
    for target, type_code, post, phase in (("CH1", "CHM", "CHM_M1", "CHM"), ("CH2", "CHM", "CHM_M2", "CHM")):
        ...
        for excel_row, values in sheets[sheet_name][row_no:]:
            ...
            if not pedido:
                continue
            order = _base_order(values, indexes, excel_row, target, type_code, visual, pedido)
            order["operaciones"] = [{"fase": phase, "puesto": post}]
            orders.append(order)
...
def _period_values(values, weeks, years):
    for value in values:
        ...
        if isinstance(value, (datetime, date)): years.add(value.year)
            ...
            years.update(int(x) for x in re.findall(r"\b(20\d{2})\b", text))
...
def _parse_times(sheets):
    ...
    phases = {"CEP1": all_indexes.get("CEP1"), "CEP2": all_indexes.get("CEP2")}
    result, seen = [], set()
    for excel_row, values in sheets[actual][row_no:]:
        ...
        phase_values = {"CHM": {"CHM": values[indexes["TIEMPO"]] if indexes["TIEMPO"] < len(values) else None}, "CHE": {"CEP1": values[phases["CEP1"]] if phases["CEP1"] is not None and phases["CEP1"] < len(values) else None, "CEP2": values[phases["CEP2"]] if phases["CEP2"] is not None and phases["CEP2"] < len(values) else None}, "CHC": {"CEP1": values[phases["CEP1"]] if phases["CEP1"] is not None and phases["CEP1"] < len(values) else None, "CEP2": values[phases["CEP2"]] if phases["CEP2"] is not None and phases["CEP2"] < len(values) else None}}.get(type_code)
        if not phase_values or not model: result.append({"tipo_chasis": type_code, "modelo": model, "fase": None, "minutos": None, "fila_origen": excel_row, "error": "Modelo o tipo vacío"}); continue
